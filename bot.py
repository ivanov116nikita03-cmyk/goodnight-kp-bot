...
def parse_card(text):
    ...
    # ИНН (10 или 12 цифр — берём первый, не ИНН банка)
    for m in re.finditer(r'ИНН\D{0,10}(\d{10,12})', t, re.I):
        val = m.group(1)
        # Пропускаем ИНН банка
    ...
    # БИК (9 цифр) — "БИК банка044525974" или "БИК: 044525974"
    ...
    # Адрес — ищем с почтового индекса (6 цифр), пропускаем слова вроде "организации"
    for addr_pat in [
